 ...
 */
package com.otago.lecturercommon.interceptor;
...
import com.otago.lecturercommon.utill.CommonUtil;
...
public class AllRequestInterceptor extends HandlerInterceptorAdapter {
    private static org.apache.log4j.Logger log = Logger.getLogger(AllRequestInterceptor.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (currentUrl.startsWith("/cache") || currentUrl.startsWith("/property")) {
            return true;
        } else {
...
//			commonUtil.writeErrorResponseForPostRequest(request, response, "ERROR_REQLIMIT", String.valueOf(AfroConstant.MAX_REQUEST_FROMIP_PERMINUTE));
//			return false;
//		} else {
//			String url = commonUtil.getUrl(request);
//			String sessionId = commonUtil.checkAndSetSessionId(request, response);
//			if (!isPublicUrl(url)) {
...
//			request.setAttribute(AfroConstant.PARAM_RETURNURL, commonUtil.getReturnUrl(request));
//		}
        return true;
    }
    ...
    private String checkAuthAndUser(HttpServletRequest request) throws IOException {
        String[] urls = new String[] { "/user/myprofile", "/user/transactiondetails" };
        ...
        try {
            ...
            if (isAuthReq) {
                ...
                if (user == null) {
                    ...
                    try {
                        boolean flag = commonUtil.processUserLogin(sessionId, request);
                        if (!flag) {
//                           temp set unauthorizeduser, later it will redirect for login page
                            // redirectUrl = BackLinkUtil.HOME_PAGE_URL;
                            redirectUrl = "/";
//                          redirectUrl = commonUtil.getLoginRedirect(sessionId, Platform.TXN_VIA_WEB, request);
                        }
                    } catch (Exception e) {
